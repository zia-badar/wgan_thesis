...
from anamoly_det_test_1.FixedSeedRandomSampler import FixedSeedRandomSampler
...
def train_encoder(config):
    ...
    train_dataset = [None] * config['encoders_n']
    augmentations_sets = [None] * config['encoders_n']
    for i in tqdm(range(config['encoders_n'])):
        train_dataset[i] = PerSampleAugmentDataset(train_inlier_dataset)
        augmentations_sets[i] = train_dataset[i].get_augmentation_set()
    ...
    discriminator_dataloader_iter = [None] * config['encoders_n']
    encoder_dataloader_iter = [None] * config['encoders_n']
    fixSeedRandomSamplers = [None] * config['encoders_n']

    for i in range(config['encoders_n']):
        fixSeedRandomSamplers[i] = FixedSeedRandomSampler(train_dataset[i])
        discriminator_dataloader_iter[i] = iter(DataLoader(train_dataset[i], batch_size=config['batch_size'], sampler=fixSeedRandomSamplers[i], num_workers=config['num_workers']))
        encoder_dataloader_iter[i] = iter( DataLoader(train_dataset[i], batch_size=config['batch_size'], sampler=fixSeedRandomSamplers[i], num_workers=config['num_workers']))
    ...
    result.set_aug_transform(augmentations_sets)
    ...
    for encoder_iter in progress_bar:
        ...
        for _ in range(config['discriminator_n']):
            ...
            for i in range(config['encoders_n']):
                empty, batch = _next(discriminator_dataloader_iter[i])
                if empty:
                    discriminator_dataloader_iter[i] = iter(DataLoader(train_dataset[i], batch_size=config['batch_size'], sampler=fixSeedRandomSamplers[i], num_workers=config['num_workers']))
                    _, batch = _next(discriminator_dataloader_iter[i])
                ...
                loss = -torch.mean(fs[i](z) - fs[i](es[i](norm_transform(x))))
        ...
        e_xs = []
        for i  in range(config['encoders_n']):
            empty, batch = _next(encoder_dataloader_iter[i])
            if empty:
                encoder_dataloader_iter[i] = iter(DataLoader(train_dataset[i], batch_size=config['batch_size'], sampler=fixSeedRandomSamplers[i], num_workers=config['num_workers']))
                _, batch = _next(encoder_dataloader_iter[i])

            x, _ = batch
            x = x.cuda()

            e_xs.append(es[i](norm_transform(x)))
        ...
        for i in range(config['encoders_n']):
            f_x = fs[i](e_xs[i])
            l1 = torch.mean(f_x)
            l2 = 1e-2*torch.mean(cosine_sim(center, e_xs[i]))
            loss = -(l1 + l2)
            # loss = -torch.mean(f_x)
        ...
        if encoder_iter % 1000 == 0:
            ...
            covs = []
            for i in range(config['encoders_n']):
                mean, cov = evaluate_encoder(es[i], train_dataset[i], validation_dataset, config, norm_transform)
                means.append(mean)
                covs.append(cov)
...
def evaluate_encoder(encoder, train_dataset, validation_dataset, config, norm_transform):
    encoder.eval()
    ...
    with torch.no_grad():
        for dataset in [train_dataset]:
            ...
            for x, _ in train_dataloader:
                x = x.cuda()
                encodings.append(encoder(norm_transform(x)))
...
if __name__ == '__main__':
    ...
    for _class in range(10):
        ...
        config = {'batch_size': 64, 'epochs': 200, 'encoding_dim': 32, 'encoder_iters': 5000, 'discriminator_n': 1, 'lr': 5e-5, 'weight_decay': 1e-6, 'clip': 1e-2, 'num_workers': 20, 'result_folder': f'results/set_{(int)(mktime(localtime()))}_{_class}/', 'encoders_n': 2 }
        config['lambda'] = 0
