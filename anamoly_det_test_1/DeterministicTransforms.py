...
class ColorJitter(torch.nn.Module):
    ...
    def forward(self, img):
        ...
        # )
        if not hasattr(self, 'fn_idx'):
            self.fn_idx, self.brightness_factor, self.contrast_factor, self.saturation_factor, self.hue_factor = self.get_params(
            self.brightness, self.contrast, self.saturation, self.hue
        )
...
class RandomResizedCrop(torch.nn.Module):
    ...
    def forward(self, img):
        ...
        if not hasattr(self, 'i'):
            self.i, self.j, self.h, self.w = self.get_params(img, self.scale, self.ratio)
        return F.resized_crop(img, self.i, self.j, self.h, self.w, self.size, self.interpolation, antialias=self.antialias)
